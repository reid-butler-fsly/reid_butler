...
def test_input_operations(capsys: pytest.fixture) -> None:
    ...
    input_name_project_id = (
        f"projects/se-development-9566/locations/us-west2/inputs/fastly-reid-inputlocations/us-west2/inputs/fastly-reid-input"
    )

    # Tests

    response = create_input.create_input(project_name, location, f"fastly-reid-input-{uuid.uuid4()}")
    ...
    response = update_input.update_input(project_name, location, f"fastly-reid-input-{uuid.uuid4()}")
    assert input_name_project_id in response.name
    assert response.preprocessing_config.crop.top_pixels == 5

    response = get_input.get_input(project_name, location, f"fastly-reid-input-{uuid.uuid4()}")
    assert input_name_project_id in response.name

    response = delete_input.delete_input(project_name, location, f"fastly-reid-input-{uuid.uuid4()}")
